...
| Path      | Page                                                                                                    |
| --------- | ------------------------------------------------------------------------------------------------------- |
| `/`       | Redirects to `/overview` for signed-in users; guests hit the app shell and go to `/login` |
...
- Canvas: a seamless light or dark tonal background across both columns, with concise product copy and a slow gray waveform inspired by bill and spending flows. The line field continues quietly behind the form, and reduced motion keeps it static.
...
- Geist carries all auth typography. Primary inverts against the canvas (near-black on light, near-white on dark); surfaces and hierarchy stay charcoal and flat.
- Header: the Kosh mark sits at the far left inside the desktop brand area. It moves to the top-left of the form column only when the brand area is hidden on smaller screens.
...
- Fields: use the standard shadcn structure with visible labels above each control. Keep auth-specific focus treatment local to the auth components: muted rings in both themes. Place one concise validation message below the related field.
